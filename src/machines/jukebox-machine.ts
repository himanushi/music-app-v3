...
>(
  {
    ...
    states: {
      idle: {
        entry: ["initMusicPlayer", "remember"],
        on: {
          PLAY_OR_PAUSE: {
            // remember 実行後のみ許可する
            target: "loading",
    ...
    on: {
      ...
      MOVE: { actions: ["moveTracks", "memory"] },
      ...
      REMOVE: { actions: ["removeTrack", "memory"] },

      REPEAT: { actions: ["repeat", "memory"] },
  ...
  },
  {
    actions: {
      ...
      setLink: assign({
        link: (_, event) => "link" in event ? event.link : "/",
      }),
